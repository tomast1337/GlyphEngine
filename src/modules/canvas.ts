...
export default class Canvas {
  ...
  constructor(sourceCanvas?: HTMLCanvasElement) {
    ...
    if (!this.canvas) {
      throw new Error(
        "Canvas 2d context is not supported on this browser.\nCan’t sort chars for brightness."
      );
    }
  ...
  resize(dWidth: number, dHeight: number) {
  ...
  sample(sx: number, sy: number, gray: boolean = false) {
    const w = this.canvas.width;
    const h = this.canvas.height;
...
function paletteQuantize(arrayIn: RGB[], arrayOut: RGB[], palette: RGB[]) {
  arrayOut = arrayOut || [];
