...
export function main(coord: Coord, context: Context, cursor: Cursor) {
  ...
  const A =
    mode % 2 == 0
      ? atan2(centerA.y - st.y, centerA.x - st.x)
      : dist(st, centerA);
  const B =
    mode == 0 ? atan2(centerB.y - st.y, centerB.x - st.x) : dist(st, centerB);
