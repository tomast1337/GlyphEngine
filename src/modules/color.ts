...
// Convert r,g,b,a values to {r,g,b,a}
export function hex(r: number, g: number, b: number, a: number = 1.0) {
  return rgb2hex({ r, g, b, a });
}

// Convert r,g,b,a values to 'rgb(r,g,b,a)'
export function css(r: number, g: number, b: number, a: number = 1.0) {
...
// Convert {r,g,b,a} values to 'rgb(r,g,b,a)'
export function rgb2css(rgb: RGB) {
...
export function rgb2gray(rgb: RGB) {
...
const _CSS2: PaletteElement[] = [..._CSS1, { int: 0xffa500, name: "orange" }];

const _CSS3: PaletteElement[] = [
  ..._CSS2,
...
const _CSS4: PaletteElement[] = [
  ..._CSS3,
  { int: 0x663399, name: "rebeccapurple" },
];

// Helper function
function augment(pal: PaletteElement[]): AugmentedPaletteElement[] {
  return pal.map((el) => {
...
// Helper function
function toMap(
  pal: AugmentedPaletteElement[]
): Record<string, AugmentedPaletteElement> {
  const out: Record<string, AugmentedPaletteElement> = {};
  pal.forEach((el) => {
    out[el.name] = el;
  });
  return out;
